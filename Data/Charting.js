...
function start_PM() {
  ...
  for ( var i=0; i*10<data0.length; i++ ) {
    ...
    for ( var j=0; j<newPM.length; j++ ) {
        data1[i][j]= Math.round(parseFloat(newPM[j])*1000);
  ...
  for ( var i=0; i<data1.length; i++ ) {
    ...
    if ( mode=="Position") for ( var j=0; j<3; j++ ) {
      ...
      if ( i==0 ) 
        board_context.moveTo(i, HEIGHT);
      else
        board_context.moveTo(i, HEIGHT-data1[i-1][j]/2*scale);
      board_context.lineTo(i+1, HEIGHT-data1[i][j]/2*scale);
      board_context.stroke();
    }
    ...
    if ( mode=="Acceleration") for ( var j=3; j<6; j++ ) {
      ...
      if ( i==0 || i==1 ) 
        ...
        board_context.moveTo(i, HEIGHT-(data1[i-1][j]-data1[i-2][j])/2*scale);
      if ( i>1 ) board_context.lineTo(i+1, HEIGHT-(data1[i][j]-data1[i-1][j])/2*scale);
      board_context.stroke();
    }
  }
}
