...
double polyPredict ( queue <double> a, queue <double> currTime, double predictVel)
...
double polyPredict ( vector <double> a, vector <double> currTime )
{

    int i, j, k, n, size, predictPos;
    n = 2;
...
void updateHaptics(void)
{
    ...
    //comparative values for velocity
    double cx = 0.0;
    double cy = 0.0;
    double cz = 0.0;
    double px = 0.0;
    ...
    //comparative values for acceleration
    double cxAccel = 0.0;
    double cyAccel = 0.0;
    double czAccel = 0.0;
    double pxAccel = 0.0;
    ...
    vector <double> yPos;
    vector <double> yDelta;
    vector <double> zPos;
    vector <double> zDelta;

    //store predicted position here for comparisons
    vector <double> xPredictedPos;
    vector <double> yPredictedPos;
    vector <double> zPredictedPos;
    cVector3d predictedPosition;
    ...
    while(simulationRunning)
    {
        ...
        xPos.push_back(position.get(0));
        yPos.push_back(position.get(1));
        zPos.push_back(position.get(2));
        ...
        //write acceleration to logfile
        logfile << cxAccel << "X Acceleration" << cyAccel << "Y Acceleration" << czAccel << "Z Acceleration\n";

        //clear vectors when curve changes (i.e. inflection point)
        if (signbit(cxAccel)!=signbit(pxAccel)){
            //clear x vectors
            xPos.clear();
            xDelta.clear();
        }
        if (signbit(cyAccel)!=signbit(pyAccel)){
            //clear y vectors
            yPos.clear();
            yDelta.clear();
        }
        if (signbit(czAccel)!=signbit(pzAccel)){
            //clear z vectors
            yPos.clear();
            yDelta.clear();
        }

        //set new prevous velocity and acceleration values
        px = cx;
        py = cy;
        pz = cz;
        pxAccel = cxAccel;
        ...
        // desired position
        predictedPosition.set(xPredictedPos.back(),yPredictedPos.back(),zPredictedPos.back());
        ...
        if (useForceField)
        {
            ...
            cVector3d forceField = haptics_level * (predictedPosition - position);
        ...
        /////////////////////////////////////////////////////////////////////
        // UPDATE TIMERS AND POPULATE TIME VECTORS
        /////////////////////////////////////////////////////////////////////
        ...
        xDelta.push_back(timeElapsed);
        yDelta.push_back(timeElapsed);
        zDelta.push_back(timeElapsed);

        //dequeue position and time values
