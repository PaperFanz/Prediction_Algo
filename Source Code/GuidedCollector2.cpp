...
void keySelect(unsigned char key, int x, int y)
{
    ...
    // options w and s
    if (key == 'w'){
        haptics_scale = haptics_scale + 80.0;
    }
    if (key == 's'){
        haptics_scale = haptics_scale - 80.0;
    }
...
void updateHaptics(void)
{
    ...
    for ( int count=0; simulationRunning; count++ )
    {
        ...
        if (count%50 == 0){
            ...
            guide = cSub( desiredPosition, position) * haptics_level;
        }
        ...
        else
        {
            ...
            cVector3d limiter = cSub(linearVelocity, normalizedVelocity*ideal) * haptics_level;
            force.add(limiter);
        }
