...
void keySelect(unsigned char key, int x, int y)
{
    ...
    // options w and s
    if (key == 'w'){
        haptics_scale = haptics_scale + 10.0;
    }
    if (key == 's'){
        haptics_scale = haptics_scale - 10.0;
    }
...
void updateHaptics(void)
{
    ...
    for ( int count=0; simulationRunning; count++ )
    {
        ...
        haptics_level = ( ideal - linearVelocity.length() ) * haptics_scale;
        ...
        double min = 0.0;
        srand(time(0));
        randX = (max - min) * ( (double)rand() / (double)RAND_MAX ) + min;
        randY = (max - min) * ( (double)rand() / (double)RAND_MAX ) + min;
        randZ = (max - min) * ( (double)rand() / (double)RAND_MAX ) + min;
        cout << randX;
        random.set(randX,randY,randZ);
        force.add(random);
