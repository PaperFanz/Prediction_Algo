...
double delay = 1.0; //delay in seconds
...
void keySelect(unsigned char key, int x, int y)
{
    ...
    if (key == 'd'){
        if (delay -.1 >= 0){
            ...
            cout << "Delay cannot be smaller than 0!\n";
        }
    }
}
...
void updateGraphics(void)
{
    ...
    labelDelay->setText( cStr(delay) );

    // update position of label
    labelDelay->setLocalPos(20, windowH - 80, 0);
...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        if ( xPos.size() >= delay * 1000){
            position.set(xPos.front(),yPos.front(),zPos.front());
