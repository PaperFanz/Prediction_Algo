...
int main(int argc, char* argv[])
{
    ...
    cout << "-----------------------------------" << endl;
    cout << "CHAI3D" << endl;
    cout << "Prediction Algorithm v3" << endl;
    cout << "-----------------------------------" << endl << endl << endl;
    ...
    cout << "[w] - increase haptics level" << endl;
    cout << "[s] - decrease haptics level" << endl;
    cout << endl << endl;
...
void keySelect(unsigned char key, int x, int y)
{
    ...
    if (key == "r"){
        if (position_lock = 1.0){
            position_lock = 0.0;
        }
        if (position_lock = 0.0){
            position_lock = 1.0;
        }
    }
}
...
void updateGraphics(void)
{
    ...
    labelHapticLevel->setText( "Haptics level:" + haptics_level );

    // update position of label
    labelHapticLevel->setLocalPos(20, windoH - 80, 0);
...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        accel.set((cx-px)*.1, (cy-py)*.1, (cz-pz)*.1);
        accel = accel * .1;
        ...
        desiredPosition = cAdd(position, cAdd(linearVelocity, accel)) * position_lock;
        ...
        if (useForceField)
        {
            ...
            cVector3d forceField = haptics_level * (desiredPosition - position) * haptics_thresh;
