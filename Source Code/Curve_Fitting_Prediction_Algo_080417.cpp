...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        //enqueue position values
        xPos.push (position.get(0));
        yPos.push (position.get(1));
        zPos.push (position.get(2));
        ...
        avgLinearVelocity = avgLinearVelocity * preTime * scale;

        //acceleration calculations
        accel.set((cx-px), (cy-py), (cz-pz));
        accel = accel * preTime * scale;

        //set predicted velocity
        predictVel = cAdd(avgLinearVelocity, accel)

        //set new prevous velocity values
        px = cx;
        py = cy;
        pz = cz;
        ...
        velocity->m_pointB = cAdd(position, predictVel);
        ...
        predictIndicator->setLocalPos(cAdd(position, predictVel));
